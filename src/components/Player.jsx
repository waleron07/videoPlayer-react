...
const Player1 = () => {
  ...
  useEffect(() => {
    setVideo(inputRef);
    if (video) {
      ["paused", "play"].forEach((event) => {
        video.current.addEventListener(event, () => handleUpdate());
      });
  ...
  const scrub = (event) => {
    const scrubTime =
      (event.nativeEvent.offsetX / video.current.clientWidth) *
      ...
      <div className="controls">
        ...
        <div className="menu__buttons">
          <button
            ...
            onClick={togglePlay}
          >
            <svg className="svg_play_pause" viewBox="0 0 232.153 232.153">
